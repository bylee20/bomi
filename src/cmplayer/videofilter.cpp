...
bool VideoFilter::pass(VideoFrame &in, QLinkedList<VideoFrame> &queue, double /*prevPts*/) {
	queue.push_back(in);
	return true;
}
...
bool SoftwareDeinterlacer::tryGraph(const VideoFrame &in, QList<mp_image*> &outs) {
	if (m_type != Graph)
		return false;
	if (!m_graph.initialize(m_option, in.format().size(), in.format().imgfmt()))
		return false;
	if (!m_graph.push(in.mpi()))
		return false;
	while (auto out = m_graph.pull())
		outs.push_back(out);
	return !outs.isEmpty();
}

bool SoftwareDeinterlacer::tryPostProc(const VideoFrame &in, QList<mp_image*> &outs) {
	if (m_type != PP)
		return false;
	if (!m_pp.initialize(m_option, in.format().size(), in.format().imgfmt()))
		return false;
	auto topField = [this] (mp_image *in) {
		auto out = m_pp.newImage(in);
		m_pp.process(out, in);
		return out;
	//	paintOut(img, mpi);
	};
	auto bottomField = [this] (mp_image *in) {
		auto out = m_pp.newImage(in);
		 in->planes[0] +=  in->stride[0];
	...
	outs.push_back(topFirst ? topField(in.mpi()) : bottomField(in.mpi()));
	if (m_deint.doubler)
		outs.push_back(!topFirst ? topField(in.mpi()) : bottomField(in.mpi()));
	return true;
}
...
void SoftwareDeinterlacer::split(const VideoFrame &in, QLinkedList<VideoFrame> &outs, double prev) {
	static const VideoFrame::Field field[] = {VideoFrame::Bottom, VideoFrame::Top};
	const bool topFirst = in.mpi()->fields & MP_IMGFIELD_TOP_FIRST;
	outs.push_back(VideoFrame(false, in.mpi(), field[topFirst] | in.field()));
	if (m_deint.doubler)
		outs.append(VideoFrame(false, in.mpi(), in.pts() + ptsStep(in.pts(), prev), field[!topFirst] | VideoFrame::Additional | in.field()));
}

void SoftwareDeinterlacer::process(const VideoFrame &in, QLinkedList<VideoFrame> &queue, double prev) {
	Type type = (in.mpi()->fields & MP_IMGFIELD_INTERLACED) ? m_type : Pass;
	QList<mp_image*> outs;
	switch (type) {
	case Graph: tryGraph(in, outs);    break;
	case PP:    tryPostProc(in, outs); break;
	case Mark: split(in, queue, prev);return;
	default:                           break;
	}
	if (outs.isEmpty())
		queue.push_back(in);
	else {
		Q_ASSERT(!outs.isEmpty());
		double step = 0.0;
		if (prev != MP_NOPTS_VALUE) {
			step = (in.pts() - prev)/double(outs.size());
			if (!(0.0 < step && step < 0.5))
				step = 0.0;
		}
		for (int i=0; i<outs.size(); ++i) {
			...
			mpi->pts = in.pts() + step*(double)i;
			queue.push_back(VideoFrame(true, mpi, in.field()));
		}
	}
}

void SoftwareDeinterlacer::setOption(DeintOption deint) {
	if (!_Change(m_deint, deint))
		return;
	m_option.clear();
	if (m_deint.method == DeintMethod::None) {
		m_type = Pass;
	} else if (deint.hwacc) {
		m_type = Mark;
	} else {
		m_type = PP;
		switch (m_deint.method) {
		...
		case DeintMethod::Yadif:
			m_option = _L("yadif") + (m_deint.doubler ? "=mode=1" : "");
			m_type = Graph;
			break;
		default:
			m_type = Pass;
