...
struct VideoFormat {
	...
	VideoFormat() {
		fourcc = bpp = width = height = stride = 0;
	}
	inline bool isCompatibleWith(const VideoFormat &other) const {
		return fourcc == other.fourcc && bpp == other.bpp && other.height < height && other.stride < stride;
	}
	inline bool operator == (const VideoFormat &rhs) const {
		return fourcc == rhs.fourcc && bpp == rhs.bpp && width == rhs.width && height == rhs.height && stride == rhs.stride;
	}
