...
Item {
    ...
    Binding { target: player; property: "height"; value: Util.fullScreen ? root.height : root.height - controls.height }
    Connections {
        target: Util
        onFullScreenChanged: {
            catcher.update()
    ...
    Component.onCompleted: {player.parent = root; controls.parent = catcher}
}
