...
auto SubSearchPathModel::header(int column) const -> QString
{
    switch (column) {
    case 0:
...
auto SubSearchPathModel::displayData(int r, int c) const -> QVariant
{
    switch (c) {
    case 0:
        return at(r).string();
...
auto SubSearchPathModel::edit(int row, int column, const QVariant &var) -> bool
{
    auto &t = get(row);
    switch (column) {
    case 0: {
