...
//VideoFrame::VideoFrame::Data::Data(mp_image *mpi, int field)
//: format(mpi), pts(mpi->pts), field(field) {
//    data[0] = mpi->planes[0];
//    data[1] = mpi->planes[1];
//    data[2] = mpi->planes[2];
//    data[3] = mpi->planes[3];
...
//auto VideoFrame::toImage() const -> QImage
//{
//    if (!d->image.isNull())
//        return d->image;
...
//    QImage image(d->format.imageSize(), QImage::Format_RGB888);
...
//auto VideoFrame::allocate(const VideoFormat &format) -> void
//{
...
//auto VideoFrame::doDeepCopy(const VideoFrame &frame) -> void
//{
//    d.detach();
