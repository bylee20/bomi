...
//void SubtitleRenderer::load(const QList<Loaded> &loaded) {
//	unload();
//	d->loaded = loaded;

//	for (const Loaded &loaded : d->loaded) {
//		if (loaded.isSelected())
//			d->order.prepend(new Render(loaded.component()));
//	}
//	qSort(d->order.begin(), d->order.end(), [this] (const Render *lhs, const Render *rhs) {
